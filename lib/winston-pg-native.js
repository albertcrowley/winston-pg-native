...
class Postgres extends Transport {
  ...
  log(args, callback) {
    ...
    try {
      let { level, message, meta } = args
      if (!callback) {
        callback = () => {
        }
      }

      // copy in any data that we get which isn't in the meta variable already.
      if (!meta) {
        meta = {}
      }
      for (let key in args) {
      ...
      try {
        ...
        meta_string = stringify(meta)
      }

      if (sqlStatement) {
        sqlStatement = {
          text: sqlStatement,
          values: ['now()', level, message, meta_string]
        }
      }
      ...
      return pool.connect().then(client => client
        .query(sqlStatement || table.insert(table.timestamp.value('NOW()'), table.level.value(level), table.message.value(message), table.meta.value(meta_string)).toQuery())
        .then(() => {
          client.release()
      ...
      console.log("ERROR in postgres log ")
      console.log(e)
    }
